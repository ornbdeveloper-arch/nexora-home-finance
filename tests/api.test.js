...
test('API exige autenticação e isola todo o CRUD por usuário', async () => {
  ...
  try {
    ...
    const petsId = a.categories.at(-1).id;
    a = (await api('/transactions/' + aId, 'token-a', 'PUT', { ...transaction, amount: 1500, status: 'paid', category: petsId })).data;
    a = (await api('/budgets', 'token-a', 'PUT', { month: '2026-09', category: petsId, amount: 10000 })).data;
    a = (await api('/installments', 'token-a', 'POST', { description: 'Notebook', totalAmount: 600000, installmentCount: 12, startMonth: '2026-09', dueDay: 10, category: petsId })).data;
